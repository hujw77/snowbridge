...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		type CreateAsset: CreateAsset<
			Self::AccountId,
			<Self::Assets as Inspect<Self::AccountId>>::AssetId,
			<Self::Assets as Inspect<Self::AccountId>>::Balance,
		>;

		type Assets: Mutate<Self::AccountId, Balance = u128, AssetId = u128>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create(origin: OriginFor<T>, token: H160) -> DispatchResult {
			...
			if Self::asset_id(token).is_some() {
				return DispatchError::Token(TokenError::CannotCreate).into();
			}

			let asset_id = Self::make_asset_id(token);
			T::CreateAsset::create(asset_id, &T::PalletId::get().into_account(), true, 1);
