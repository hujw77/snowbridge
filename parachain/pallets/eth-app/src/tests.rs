use crate::mock::{
	new_tester, AccountId, Assets, Ether, EtherApp, EtherAppPalletId, Event, Origin, System, Test,
};
...
fn mints_after_handling_ethereum_event() {
	new_tester().execute_with(|| {
		...
		// TODO: Rather create asset using genesis config
		assert_ok!(Assets::force_create(
			frame_system::RawOrigin::Root.into(),
			0,
			EtherAppPalletId::get().into_account(),
			true,
			1
		));
...
fn should_not_burn_on_commitment_failure() {
	new_tester().execute_with(|| {
		...
		assert_noop!(
			EtherApp::burn(Origin::signed(sender.clone()), ChannelId::Basic, recipient.clone(), 20),
